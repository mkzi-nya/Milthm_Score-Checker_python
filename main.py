        ...
        with open("./beatmapid字典.txt", "r", encoding="utf-8") as f:
            for ln in f:
                ...
                if not ln or ln.startswith("#"):
                    continue
                m = re.match(
                    r'"([^"]+)":\s*\{\s*constant:\s*([-\d\.]+)\s+category:\s*"([^"]+)"\s+name:\s*"([^"]+)"\s*\},?',
                    ln,
                )
                if m:
                    d[m.group(1)] = {
...
def reality(score):
    if score >= 1005000:
        return 1
    if score >= 995000:
        return 1.4 / (math.exp(-3.65 * (score / 10000 - 99.5)) + 1) - 0.4
    if score >= 980000:
        return (
            (math.exp(3.1 * (score - 980000) / 15000) - 1) / (math.exp(3.1) - 1)
        ) * 0.8 - 0.5
    if score >= 700000:
        return score / 280000 - 4
...
def regFile(s):
    m = re.search(
        r'"PlayerFile(?:_h\d+)?"\s*=\s*hex:((?:[0-9a-fA-F]{2},?[\s\\\n]*)+)', s
    )
    if m:
        hd = re.sub(r"[,\\\s\n]", "", m.group(1))
...
def pOld(s):
    ...
    j += 3
    d = tJ(s[i:j])
    if not d or "SongRecords" not in d:
        return None, []
    u = d.get("UserName")
    its = []
...
def avgR(its):
    arr = sorted(
        [x["singleRealityRaw"] for x in its if x["singleRealityRaw"] > 0], reverse=True
    )
...
def drawImg(its, uname, uR, outp, drawCount):
    ...
    def draw_gradient_txt(cv, xy, txt, ft, tc, bc):
        ...
        for y in range(th):
            r = (
                int(tc[0] * (1 - y / (th - 1)) + bc[0] * (y / (th - 1)))
                if th > 1
                else tc[0]
            )
            gg = (
                int(tc[1] * (1 - y / (th - 1)) + bc[1] * (y / (th - 1)))
                if th > 1
                else tc[1]
            )
            bbb = (
                int(tc[2] * (1 - y / (th - 1)) + bc[2] * (y / (th - 1)))
                if th > 1
                else tc[2]
            )
            for x in range(tw):
                g.putpixel((x, y), (r, gg, bbb, 255))
    ...
    for i in range(drawCount):
        ...
        dr.text(
            (x + cardW - 10 - (nb[2] - nb[0]), y + int(scale)),
            numT,
            fill=(250, 250, 250) if i < 20 else (201, 201, 201),
            font=fN,
        )
        ...
        if lv < 2:
            draw_gradient_txt(
                can, scPos, scT, fS, (0x99, 0xC5, 0xFB), (0xD8, 0xC3, 0xFA)
            )
        else:
            dtb(
                dr,
                ...
                scT,
                fS,
                (144, 202, 239) if lv == 2 else (255, 255, 255),
            )
        sn = its[i]["name"]
        fw = int(19 * scale)
        ...
        rt = "%s %.1f > %.2f   %.2f%%" % (
            its[i]["category"],
            its[i]["constant"],
            its[i]["singleRealityRaw"],
            its[i]["bestAccuracy"] * 100,
        )
...
def main():
    ...
    drawCount = 22
    savePath = (
        os.path.join(sd, "save.json")
        if os.path.exists(os.path.join(sd, "save.json"))
        else os.path.join(sd, "save.txt")
    )
    outPath = os.path.join(
        sd, "output_py_%s.png" % datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    )
    ...
    if len(args) > idx:
        ...
        if os.path.isdir(p):
            outPath = os.path.join(
                p,
                "output_py_%s.png" % datetime.datetime.now().strftime("%Y%m%d_%H%M%S"),
            )
        else:
            # 上级目录不存在就不处理，默认逻辑
