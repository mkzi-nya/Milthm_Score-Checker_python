    ...
    try:
        with open("./beatmapid字典.txt","r",encoding="utf-8") as f:
            for ln in f:
                ...
                if m:
                    d[m.group(1)]={"constant":float(m.group(2)),
                                   "category":m.group(3),
                                   "name":m.group(4)}
    except:pass
    return d
...
def pXML(s):
    try:
        r=ET.fromstring(s)
        n=r.find(".//string[@name='PlayerFile']")
...
def psN(s):
    try:
        ...
        r=reality(sc)
        sr=(r+co)if r else 0
        return{"name":t.strip(),"category":c.strip(),"constant":co,
               "bestScore":sc,"bestAccuracy":ac,"bestLevel":l,
               "singleRealityRaw":sr}
    except:return
...
def psO(r):
    ...
    c=csts[b];sc=r.get("BestScore",0);ac=r.get("BestAccuracy",0.0);lv=r.get("BestLevel",0)
    rr=reality(sc)
...
def drawImg(its,uname,uR,outp,drawCount,drawLevel):
    ...
    dr.text((100,90),"Reality查分器v1.2",fill=(255,255,255),font=f50)
    ...
    for i in range(drawCount):
        ...
        bg=Image.new("RGBA",(cardW,cardH),(128,128,128,int(0.3*255)))
        can.paste(bg,(x,y),bg)
        numT="#"+str(i+1)
        nb=dr.textbbox((0,0),numT,font=fN)
        dr.text((x+cardW-10-(nb[2]-nb[0]),y+int(scale)),
                numT,fill=(250,250,250) if i<20 else (201,201,201),font=fN)
        scT=str(its[i]["bestScore"]).zfill(7)
        scPos=(int(x+160*scale),int(y+53*scale))
        lv=its[i]["bestLevel"]
        if lv<2:
            draw_gradient_txt(can,scPos,scT,fS,(0x99,0xC5,0xFB),(0xE8,0xD3,0xFF))
        else:
            dtb(dr,scPos[0],scPos[1]-5,scT,fS,(154,218,250) if lv==2 else (255,255,255))
        sn=its[i]["name"]
        fw=int(19*scale)
        ...
        while dr.textbbox((0,0),sn,font=fSong)[2]>200 and fw>10:
            fw-=1
            fSong=ImageFont.truetype(fp,fw,index=2)
        dtb(dr,int(x+163*scale),int(y+24*scale),sn,fSong,(255,255,255))
        ...
        if drawLevel==1:
            if lv>=6:levelFile="6.png"
                ...
                if lv in [1,2,3,4,5]:levelFile=f"{lv}.png"
                else:levelFile=None
            if levelFile:
                lvlPath=os.path.join(sd,"jpgs",levelFile)
...
def load_links(txt_path="./links.txt"):
    ...
    with open(txt_path,"r",encoding="utf-8") as f:
        for line in f:
            ...
            if m:
                lm[m.group(1).strip()]=m.group(2).strip()
    return lm
...
def download_with_progress(url, out_path):
    r=requests.get(url,stream=True);r.raise_for_status()
    total=r.headers.get('Content-Length');total=int(total)if total else 0
    downloaded=0
    with open(out_path,"wb")as f:
        for c in r.iter_content(chunk_size=8192):
            if c:
...
def download_all_parallel(links_map):
    ...
    with ProcessPoolExecutor()as exe:
        ...
        for fut in as_completed(fut_map):
            ...
            if r:
                done_cnt+=1
                if show_progress:print("[OK]",r)
    if done_cnt>0 and show_progress:
        with open(sentinel,"w")as f:f.write("downloaded.\n")
...
def main():
    ...
    if len(args)>0 and args[0].isdigit():
        drawLevel=int(args[0]);args=args[1:]
    drawCount=22
    if len(args)>0 and args[0].isdigit():
        drawCount=int(args[0]);args=args[1:]
    ...
    if len(args)>idx:
        sp=args[idx]
        if os.path.exists(sp):savePath=sp
        idx+=1
    if len(args)>idx:
        p=args[idx]
        if os.path.isdir(p):
            ...
            if dirp==""or os.path.isdir(dirp):outPath=p
    lm=load_links("./links.txt")
