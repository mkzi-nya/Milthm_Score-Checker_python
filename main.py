...
# 使用环境变量 REALITY_SCRIPT_DIR，如果未设置则回退为当前脚本所在目录
script_dir = os.getenv("REALITY_SCRIPT_DIR", os.path.dirname(os.path.abspath(__file__)))
...
def drawImg(its, uname, uR, outp, drawCount, drawLevel):
    ...
    for i in range(drawCount):
        nm=its[i]["name"]
        ip=os.path.join(script_dir, "jpgs", nm+".jpg")
        if not os.path.exists(ip):
            ip=os.path.join(script_dir, "jpgs", "NYA.jpg")
        try:
    ...
    for i in range(drawCount):
        ...
        can.paste(ci,(int(x+10*scale),int(y+10*scale)), ci)
...
def load_links(txt_path=None):
    if not txt_path:
        txt_path = os.path.join(script_dir, "links.txt")
    lm={}
